...
import { 
  ...
  FaEdit, FaTrashAlt, FaSave, FaCloudUploadAlt, FaCircle, FaTimes 
} from 'react-icons/fa';
import './Category.css'
	...
	// Fetch category data to edit
		const fetchCategoryData = async () => {
			try {
				...
				const categoryRes = await fetchDataFromApi(`/api/v1/categories/${id}`);
        ...
        .filter(Boolean); // Remove any null values
				...
				setFormData({
					...
					parent: categoryRes.data?.parent?._id || '', // Set current parent if exists
					description: categoryRes.data?.description || '',
					displayOrder: categoryRes.data?.displayOrder || 0,
					isFeatured: categoryRes.data?.isFeatured || false,
					image: {
						url: categoryRes.data?.image?.url || '',
						altText: categoryRes.data?.image?.altText || ''
					},
					seo: {
						title: categoryRes.data?.seo?.title || '',
						metaDescription: categoryRes.data?.seo?.metaDescription || '',
						keywords: categoryRes.data?.seo?.keywords || []
					},
					customFields: categoryRes.data?.customFields || {}
				});

				// Set available parent categories in state
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitData = {
	    ...
	    clearImages()
      // Make API call
      const response = await updateDataToApi(`/api/v1/categories/${id}`, submitData)

      if (response.success) {
        // Show success message
        context.showNotification(`Category uploaded successfully!`, 'success');
		...
		context.showNotification(`Couldn't upload category, try again!`, 'error');
        throw new Error(response.message || 'Operation failed');
      }
    } catch (error) {
      	console.error('Error submitting category:', error);
		context.showNotification(`Error uploading category!`, 'error');
            ...
            <div className="dashboard-section">
                ...
                <form onSubmit={handleSubmit}>
                    ...
                    <div className="form-group">
                        ...
                        onChange={handleChange}
                        >
                        <option value={formData?.parent?.id}>{formData?.parent?.name}</option>
                        {categories.map(cat => ( 
