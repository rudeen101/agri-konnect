...
// router.get('/sub/:orderId', verifyToken, authorize(["admin"]), async (req, res) => {
//     try {
//         const orderNumber = req.params.orderId;
//         const subOrders = await SubOrder.find({orderNumber}).populate("orderedBy", "_id name");

//         if (!subOrders) return res.status(404).json({ error: 'Order not found' });
...
router.put("/paymentConfirmation", verifyToken, async (req, res) => {
    try {
        ...
        //     { _id: orderId, "paymentStatus": status }, // Find order containing the item
        // );
